class World {
  constructor(gui, generation_label, gui_size, cell_count, simulation_delay){
    this.gui = gui;
  ...
  simulate(delay){
    ...
    var that = this;
    this.simulation = setInterval(function(){that.next_generation()}, delay);
    this.gui.style.border = "solid lightgreen 2px";
  }

  update_world(gui_size, cell_count){
    this.generation_label.innerText = 0;
    this.gui_size = gui_size;
    this.cell_count = cell_count;
    this.setup_world();
  }
  ...
  generate_cells(){
    ...
    for(let y = 0; y < this.cell_count; y++){
      ...
      for(let x = 0; x < this.cell_count; x++){
        ...
        rectangle.className = "cell dead-cell";
        rectangle.style.top = (y * (this.gui_size / this.cell_count));
        rectangle.style.left = (x * (this.gui_size / this.cell_count));
        rectangle.style.width = (this.gui_size / this.cell_count - 1);
        rectangle.style.height = (this.gui_size / this.cell_count - 1);
  ...
  next_generation(){
    ...
    this.generation_label.innerText = parseInt(this.generation_label.innerText) + 1;
  }
}
